import json
import tempfile
import pytest
import torch
import pandas as pd
import selfies as sf
from pathlib import Path
from chempleter.model import ChempleterModel


@pytest.fixture
def sample_seflies_csv(tmp_path):
    "create a temp selfies csv"

    selfies_dict = {"selfies":["[C][C][O]",
            "[C][=C][C][=C][C][=C][Ring1][=Branch1]",
            "[C][C][=Branch1][C][=O][O]",
            "[C][C][C][C][C][C][Ring1][=Branch1]"]}
    
    seflies_df = pd.DataFrame(selfies_dict)
    csv_path = tmp_path / "selfies.csv"
    seflies_df.to_csv(csv_path,index=False)

    return csv_path

@pytest.fixture
def sample_stoi_file(tmp_path,sample_seflies_csv):
    "create a temp selfies csv"

    selfies_list = pd.read_csv(sample_seflies_csv)["selfies"].to_list()
    alphabet = sf.get_alphabet_from_selfies(selfies_list)
    sample_stoi = {j:i for i,j in enumerate(["[PAD]", "[START]", "[END]", "[MASK]", "[BRIDGE]"]+list(alphabet))}

    stoi_file = tmp_path / "stoi.json"

    with open(stoi_file,"w") as f:
        json.dump(sample_stoi,f)

    return stoi_file

@pytest.fixture
def sample_stoi(tmp_path,sample_stoi_file):
    "create a temp selfies csv"

    with open(sample_stoi_file,"r") as f:
        sample_stoi = json.load(f)
    
    return sample_stoi