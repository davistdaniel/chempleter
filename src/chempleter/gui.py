...
def _validate_smiles(smiles):
    try:
        prompt = handle_prompt(
            smiles, selfies=None, stoi=stoi, alter_prompt=alter_prompt_checkbox.value
        )
        return True, prompt
...
def show_generated_molecule():
    ...
    # generate
    generated_molecule, generated_smiles, generated_selfies = extend(
        model=model,
        stoi_file=stoi_file,
        itos_file=itos_file,
...
with ui.column().classes("absolute-center"):
    ...
    with ui.row():
        ...
        current_prompt = ui.label().bind_text_from(
            smiles_input, "value", backward=_validate_smiles
        )
